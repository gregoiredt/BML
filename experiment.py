...
def get_configs(c):
	if ';' in c:
		assert ' ; ' in c,  'Be careful to spaces : " ; "'
		
		list_net_configs = c.split(' ; ')
		list_configs = [list(map(int, x.split(', '))) for x in list_net_configs]
	else:
		list_configs = list(map(int, c.split(', ')))
	return list_configs

def get_alpha(alpha):
	if ',' in alpha:
		print('Multiple alphas detected')
		res = [float(x) for x in alpha.split(', ')]
		return res
	else:
		return float(alpha)
...
def get_parser():
	...
	parser.add_argument('--net_type', type=str, required=False, default='fc', help = 'Type of network : fc/cnn')

	parser.add_argument('--net_config', type=lambda x: get_configs(x))
	...
	parser.add_argument('--alpha', type=lambda x: get_alpha(x),required =False, default=0.5, help="Dirichlet distribution constant used for data partitioning")
...
def load_mnist_data(datadir):
	'''
	Out of partition data : 

	train : 60000  x 28 x 28 images
	...
	if partition == "homo":
		...
		while min_size < 10:
			...
			for k in range(K):
				...
				if isinstance(alpha, list):
					assert len(alpha)==n_nets, "The size of alpha must be equal to n_nets"
					proportions_nets = np.random.dirichlet(np.repeat(alpha, n_nets))
					props = np.array(alpha)
					proportions = np.array([proportions_nets[i] * props[i] for i in range(len(alpha))])
				else:
...
def init_nets(net_configs, dropout_p, n_nets, net_type):
	if not(isinstance(net_configs[0], list)):
		print('Unique net configuration detected !')
		...
		for net_i in range(n_nets):
			if net_type == 'fc':
				...
				net = CNNet(input_size, hidden_sizes, output_size, dropout_p)
			else:
				raise ValueError('Net type is not valid, please check help of --net_type function')

			nets[net_i] = net
	else:
		print('Multiple net widths detected !')
		output_size = net_configs[0][-1]
		input_size = net_configs[0][0]
		nets = {net_i: None for net_i in range(len(net_configs))}
		assert len(nets) == n_nets, "n_net value doesn't correspond to the number of configurations given"
		for net_i in range(len(nets)):
			hidden_sizes =  net_configs[net_i][1:-1]
			if net_type == 'fc':
				net = FcNet(input_size, hidden_sizes, output_size, dropout_p)
			elif net_type == 'cnn':
				net = CNNet(input_size, hidden_sizes, output_size, dropout_p)
...
def run_exp():
	...
	n_outputs = args.net_config[-1] if not(isinstance(args.net_config[0], list)) else args.net_config[0][-1]
	...
	for trial in range(args.trials):
		...
		nets = init_nets(args.net_config, args.dropout_p, args.n_nets, args.net_type)
		...
		if ("pdm" in args.experiment) or ("all" in args.experiment):
			...
			best_sigma0, best_sigma, best_gamma, best_test_acc, best_train_acc, best_weights, res = compute_pdm_matching_multilayer(
				nets_list, train_dl, test_dl, traindata_cls_counts, n_outputs, it=5, sigma=args.pdm_sig, sigma0=args.pdm_sig0, gamma=args.pdm_gamma
			)
			...
			for (sigma0, sigma, gamma) in product(sigma0s, sigmas, gammas):
				...
				for i, comm_round in enumerate(range(args.communication_rounds)):
					...
					conf_matrix_train, conf_matrix_test = compute_iterative_pdm_matching(
						iter_nets_list, train_dl, test_dl, traindata_cls_counts, n_outputs,
